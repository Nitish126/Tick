...
function MotoKeeperApp() {
   ...
   const handleAuthSubmit = async () => {
      try {
         ...
         const res = await axios.post(`${GATEWAY_URL}${endpoint}`, payload);
         finalizeAuth(res.data);
      } catch (e) {
         Alert.alert("Authentication Failed", e.response?.data?.error || e.message);
      } finally {
         setLoading(false);
      }
   };
   ...
   const handleGoogleLogin = async (idToken) => {
      try {
         setLoading(true);
         const res = await axios.post(`${GATEWAY_URL}/api/auth/google`, { idToken });
         finalizeAuth(res.data);
      } catch (e) {
         Alert.alert("Google Auth Failed", e.response?.data?.error || e.message);
      } finally {
         setLoading(false);
      }
   };
   ...
   const handleAppleLogin = async () => {
      try {
         ...
         const res = await axios.post(`${GATEWAY_URL}/api/auth/apple`, {
   ...
   const finalizeAuth = async (data) => {
      if (data.success) {
         const { token, user } = data.data;
         await AsyncStorage.setItem('MOTO_USER_ID', user.id);
   ...
   return (
      <SafeAreaView style={[styles.root, { backgroundColor: isDarkMode ? '#000000' : C.bg }]}>
         ...
         <Modal visible={overlay === 'AUTH'} animationType="fade">
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, backgroundColor: isDarkMode ? '#000000' : C.bg, justifyContent: 'center' }}>
               <View style={{ padding: 30 }}>
                  <View style={{ alignItems: 'center', marginBottom: 50 }}>
                     <View style={{ width: 80, height: 80, borderRadius: 24, backgroundColor: C.bluePrimary, justifyContent: 'center', alignItems: 'center', marginBottom: 20, shadowColor: C.bluePrimary, shadowOpacity: 0.4, shadowRadius: 20, shadowOffset: { width: 0, height: 10 } }}>
                        <CarFront size={40} color={C.white} />
                     </View>
                     <Text style={{ fontSize: 34, fontWeight: '900', color: isDarkMode ? C.white : C.blueDark, letterSpacing: -1.0 }}>MotoKeeper</Text>
                     <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.5)' : C.textSub, fontSize: 16, fontWeight: '600', marginTop: 8 }}>{isRegistering ? 'Create your fleet account' : 'Sign in to access your vault'}</Text>
                  </View>

                  {isRegistering && (
                     <View style={{ marginBottom: 20 }}>
                        <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.4)' : C.textSub, fontSize: 11, fontWeight: '900', letterSpacing: 1.0, marginBottom: 8 }}>FULL NAME</Text>
                        <TextInput
                           style={{ backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : C.white, color: isDarkMode ? C.white : C.blueDark, padding: 18, borderRadius: 16, fontSize: 16, fontWeight: '600', borderWidth: 1, borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : C.border }}
                  ...
                  <View style={{ marginBottom: 20 }}>
                     <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.4)' : C.textSub, fontSize: 11, fontWeight: '900', letterSpacing: 1.0, marginBottom: 8 }}>EMAIL ADDRESS</Text>
                     <TextInput
                        style={{ backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : C.white, color: isDarkMode ? C.white : C.blueDark, padding: 18, borderRadius: 16, fontSize: 16, fontWeight: '600', borderWidth: 1, borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : C.border }}
                        placeholder="driver@motokeeper.app"
                        placeholderTextColor={isDarkMode ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.3)"}
                        value={authEmail}
                  ...
                  <View style={{ marginBottom: 35 }}>
                     <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.4)' : C.textSub, fontSize: 11, fontWeight: '900', letterSpacing: 1.0, marginBottom: 8 }}>PASSWORD</Text>
                     <TextInput
                        style={{ backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : C.white, color: isDarkMode ? C.white : C.blueDark, padding: 18, borderRadius: 16, fontSize: 16, fontWeight: '600', borderWidth: 1, borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : C.border }}
                        placeholder="••••••••"
                        placeholderTextColor={isDarkMode ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.3)"}
                  ...
                  <TouchableOpacity style={{ backgroundColor: C.bluePrimary, paddingVertical: 18, borderRadius: 16, alignItems: 'center', shadowColor: C.bluePrimary, shadowOpacity: 0.3, shadowRadius: 15, shadowOffset: { width: 0, height: 8 }, elevation: 5 }} onPress={handleAuthSubmit} disabled={loading}>
                     {loading ? <ActivityIndicator color={C.white} /> : <Text style={{ color: C.white, fontWeight: '900', fontSize: 16, letterSpacing: 0.5 }}>{isRegistering ? 'Create Account' : 'Authenticate'}</Text>}
                  </TouchableOpacity>

                  <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 25 }}>
                     <View style={{ flex: 1, height: 1, backgroundColor: isDarkMode ? 'rgba(255,255,255,0.1)' : C.border }} />
                     <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.4)' : C.textSub, marginHorizontal: 15, fontWeight: '800', fontSize: 12 }}>OR</Text>
                     <View style={{ flex: 1, height: 1, backgroundColor: isDarkMode ? 'rgba(255,255,255,0.1)' : C.border }} />
                  </View>

                  <TouchableOpacity style={{ backgroundColor: isDarkMode ? '#FFFFFF' : '#000000', paddingVertical: 18, borderRadius: 16, alignItems: 'center', marginBottom: 15, flexDirection: 'row', justifyContent: 'center' }} onPress={() => promptGoogleAsync()} disabled={!googleRequest || loading}>
                     <Text style={{ color: isDarkMode ? '#000000' : '#FFFFFF', fontWeight: '800', fontSize: 16 }}>Continue with Google</Text>
                  </TouchableOpacity>

                  {Platform.OS === 'ios' && (
                     <TouchableOpacity style={{ backgroundColor: '#000000', paddingVertical: 18, borderRadius: 16, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' }} onPress={handleAppleLogin} disabled={loading}>
                        <Text style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 16 }}>Sign in with Apple</Text>
                     </TouchableOpacity>
                  )}

                  <TouchableOpacity style={{ marginTop: 30, alignItems: 'center' }} onPress={() => setIsRegistering(!isRegistering)}>
                     <Text style={{ color: isDarkMode ? 'rgba(255,255,255,0.5)' : C.textSub, fontWeight: '600', fontSize: 15 }}>
                        {isRegistering ? 'Already have an account? ' : "Don't have an account? "}
                        <Text style={{ color: C.bluePrimary, fontWeight: '800' }}>{isRegistering ? 'Sign In' : 'Sign Up'}</Text>
                     </Text>
                  </TouchableOpacity>
               </View>
